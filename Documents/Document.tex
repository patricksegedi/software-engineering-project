...
\emph{Dept. of Information Systems}

\emph{Hanyang University}

\emph{Seoul, Republic of Korea}

\emph{ceh1502@hanyang.ac.kr}

Tae Hee Kim

\emph{Dept. of English Language and Literature}

\emph{Hanyang University}

\emph{Seoul, Republic of Korea}

\emph{pieceofmind@hanyang.ac.kr}

Patrick Segedi

\emph{Dept. of Computer Science}

\emph{Chalmers Unviersity of Technology}

\emph{Gothenburg, Sweden}

\emph{segedi@chalmers.se}
...
\emph{Paju, Republic of Korea}

\emph{h0dduck@hanyang.ac.kr}
...
\emph{gumannic@students.zhaw.ch}

\emph{\textbf{Abstract---In modern smart homes, voice assistants like
Alexa and Google Home serve multiple family members but treat everyone
the same way. This application combines speaker recognition with
speaker-adaptive processing to create a personalized experience for each
household member. The system identifies who is speaking, determines
their location within the house, and enforces access rules based on
whether the user is a parent or a child. To protect user privacy, all
voice data is processed locally on the device rather than being
transmitted to external servers. When multiple users issue conflicting
commands, the system resolves them according to predefined rules and
user priority levels. Our goal is to develop a voice assistant that
genuinely understands and adapts to the needs of each family member
while ensuring the privacy of their conversations.}}

\emph{\textbf{Index Terms---Voice Assistant, Speaker Recognition,
Multi-User Personalization, Smart Home, Privacy}}

\begin{longtable}{p{0.18\linewidth} p{0.22\linewidth} p{0.55\linewidth}}
\toprule
...
\hypertarget{architecture-design-implementation}{%
\section{Architecture Design \&
Implementation}\label{architecture-design-implementation}}

\emph{A. Overall Architecture}

\includegraphics[width=7.02778in,height=2.49306in]{media/image1.png}
...
/.vscode & c\_cpp\_properties.json, launch.json, settings.json & N/A (Configuration files) \\
/Document & Document.aux, Document.fdb\_latexmk, Document.fls, Document.pdf, Document.synctex.gz, Document.tex, SmartER\_Speaker.docx & N/A (Documentation files) \\
...
/src/backend/utility/repositories/test & test\_user\_repository.py & unittest, sys, pathlib, repositories.UserDboRepository \\
/src/smarterspeaker & \_\_init\_\_.py, app.py, config.py, users.JSON & pathlib, json \\
/src/smarterspeaker/Users & \_\_init\_.py, admin\_user.py, guest\_user.py, restricted\_user.py, user.py & abc, typing \\
/src/smarterspeaker/speaker & \_\_init\_\_.py, audio\_to\_text.py, speaker\_active.py, speaker\_verification.py, tts.py, voice\_recorder.py, wake\_word\_activation.py & speechbrain, sounddevice, soundfile, torch, numpy, os, pathlib, playsound, pyttsx3 \\
/src/smarterspeaker/voice\_samples & jinseo/voices (voice files), kun/voices (voice files), patrick/voices (voice files), taehee/voices (voice files) & N/A (Audio data files) \\
...
\includegraphics[width=6.51129in,height=1.0983in]{media/image2.png}
...
\includegraphics[width=7.02778in,height=2.01736in]{media/image3.png}
...
\includegraphics[width=7.02778in,height=2.26597in]{media/image4.png}
\end{quote}
...
\begin{longtable}{p{0.18\linewidth} p{0.18\linewidth} p{0.18\linewidth} p{0.18\linewidth} p{0.22\linewidth}}
\toprule
...
\includegraphics[width=4.19444in,height=3.84722in]{media/image5.png}
